        ...
        dispatcher.register((CommandManager.literal("camera-config")
            ...
            .executes((context) -> {
                for (String option : CameraMod.configSuggestions)
                    context.getSource().sendFeedback(Text.literal(option + ": " + CameraMod.configMaps.get(option).getAsString()), false);
                return 1;
            })
            .then(CommandManager.argument("option", StringArgumentType.string())
                ...
                .executes((context) -> {
                    context.getSource().sendFeedback(Text.literal(context.getArgument("option", String.class) + ": " + CameraMod.configMaps.get(context.getArgument("option", String.class)).getAsString()), false);
                    return 1;
                })
            ))
            ...
            .suggests((context,builder) -> CommandSource.suggestMatching(CameraMod.configSuggestions,builder))
                ...
                .executes((context) -> {
                    ...
                    context.getSource().sendFeedback(Text.literal(context.getArgument("option", String.class) + " set to " + CameraMod.configMaps.get(context.getArgument("option", String.class)).getAsString()), false);
                    CameraMod.CAMERA_LOGGER.info("{}: value of '{}' set to {}", context.getSource().getDisplayName().getString(), context.getArgument("option", String.class), context.getArgument("value", String.class));
                    try {
